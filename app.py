...
# Load models (use relative paths for deployment compatibility)
...
def get_gemini_response(resume_text, job_description):
    prompt = f"""
    Act like a skilled ATS in tech hiring. Evaluate this resume based on the given job description.
    Start with how much the profile matches the job description, then give a 2-3 line summary, 
    and finally list missing keywords or skills (as points only).
    {{
        "Profile Summary": ""
    }}
    Resume: {resume_text}
